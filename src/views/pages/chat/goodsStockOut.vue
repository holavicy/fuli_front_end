  ...
  <d2-container>
    <template slot="header">
        <div class="action-wrapper">
            ...
            <div class="filter-item"><span>出库日期：</span>
    ...
    <el-table :data="list" border style="width: 100%" size="mini" v-loading="loading">
        ...
        <el-table-column label="出库日期" width="180">
          <template slot-scope="scope">
            {{dayjs(scope.row.create_time).subtract(8, 'hour').format('YYYY-M-D HH:mm:ss')}}
          </template>
        </el-table-column>
        <el-table-column prop="num" label="出库数量" width="100"></el-table-column>
        <el-table-column prop="price" label="当前出库单价" width="100"></el-table-column>
        <el-table-column prop="amount" label="当前出库总价" width="100"></el-table-column>
        <el-table-column prop="change_des" label="出库说明" width="200"></el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    getReport (page) {
      ...
      console.log(data)
      this.$api.GOODS_STOCK_OUT_DETAIL_REPORT(data).then((res) => {
        this.loading = false
        this.list = res.list
    ...
    exportFile () {
      ...
      this.$api.EXPORT_ALL_GOODS_STOCK_OUT(data).then((res) => {
